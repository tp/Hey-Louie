"""The five tools the agent dispatches.

Day 1 invariant: every tool is `local=True` and mutates the in-process
`FakeLouie` directly. Day 2 flips ALL five tools to `local=False` — including
`search_music`, because the music library only exists on the iPad (no catalog
API is available server-side). This means every tool call round-trips over
the WebSocket on Day 2; see DECISIONS.md "search_music dispatches to the iPad"
for why that's interesting rather than a problem.

Tool descriptions are load-bearing: the model picks tools off the description,
not the name. Each one below is written assuming the model sees it cold.
"""
...
from backend.evals.fake_louie import ROOMS, FakeLouie

ToolHandler = Callable[[dict[str, Any], FakeLouie], Awaitable[str]]

# Shared between handler validation and tool schema enums — single source of truth.
...
class Tool:
    schema: ToolSchema
    handler: ToolHandler
    local: bool = True  # Day 1: every tool runs in-process.


class ToolRegistry:
    """Name → Tool map. The loop holds one of these per session.

    `call()` is the single error boundary: handler exceptions become
    `ToolResultBlock(is_error=True)` rather than crashing the turn. The model
    gets to see the error string and (usually) recover on the next step.
    """
    ...
    async def call(
        self, name: str, args: dict[str, Any], state: FakeLouie, tool_use_id: str
    ) -> ToolResultBlock:
        ...
        try:
            content = await tool.handler(args, state)
        except Exception as exc:  # noqa: BLE001 — model recovers from any handler error
            return ToolResultBlock(
                tool_use_id=tool_use_id,
...
# --- catalog -------------------------------------------------------------
#
# Tiny in-memory music catalog. See DECISIONS.md "search_music uses a tiny
# in-memory catalog". Day 3 leans on the dual `thriller` mapping for the
# disambiguation eval.
#
# The catalog has two views: `_ENTRIES` is the canonical list (each entry
# carries its id, type, title, and the lowercase query tokens that match it),
# and `_BY_ID` is the reverse index play_music uses to validate ids and
# fetch the canonical title.
...
async def _h_search_music(args: dict[str, Any], _state: FakeLouie) -> str:
    ...
    hits = _search_catalog(query, type_filter)
    return json.dumps(hits)


async def _h_play_music(args: dict[str, Any], state: FakeLouie) -> str:
    ...
    # The id is opaque to the model; the title is what it should narrate.
...
async def _h_control_lights(args: dict[str, Any], state: FakeLouie) -> str:
...
async def _h_set_climate(args: dict[str, Any], state: FakeLouie) -> str:
    room = _require_room(args)
...
async def _h_query_state(args: dict[str, Any], state: FakeLouie) -> str:
...
# --- schemas -------------------------------------------------------------
#
# Descriptions are written assuming the model sees one tool list per turn and
# nothing else. They name when to reach for the tool, what each arg does, and
# (where it matters) what NOT to pass.
...
QUERY_STATE = Tool(
    ...
    handler=_h_query_state,
)


def default_registry() -> ToolRegistry:
    """The Day-1 tool set. Loop callers use this; tests can build their own."""
    reg = ToolRegistry()
    for tool in (SEARCH_MUSIC, PLAY_MUSIC, CONTROL_LIGHTS, SET_CLIMATE, QUERY_STATE):
        reg.register(tool)
    return reg
